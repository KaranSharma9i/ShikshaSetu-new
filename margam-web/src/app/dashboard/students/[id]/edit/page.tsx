...
import { getStudentProfile } from '@/lib/repositories/student'
...
export default async function EditStudentPage({
  ...
  // Fetch student profile, classes and sections in parallel
  const [profile, classes, allSections] = await Promise.all([
    getStudentProfile(supabase, institutionId, studentId),
    getClasses(supabase, institutionId),
    getAllSectionsForInstitution(supabase, institutionId)
  ])

  // Tenant check / Not found check
  if (!profile) {
    notFound()
  }
  ...
  return (
    <div className="space-y-6 font-body">
      ...
      <div className="flex flex-col gap-4 border-b border-light-gray/60 pb-5">
        <div>
          <Link
            href={`/dashboard/students/${profile.id}`}
          ...
          <h1 className="text-3xl font-bold tracking-tight text-primary font-heading">Edit Profile</h1>
          <p className="text-steel-gray mt-1 text-sm font-caption">
            Update personal information, guardian details, and section allocation for {profile.full_name}.
          </p>
        </div>
      </div>

      {/* Edit Student Form Client Component */}
      <EditStudentForm
        profile={profile}
        classes={classes}
