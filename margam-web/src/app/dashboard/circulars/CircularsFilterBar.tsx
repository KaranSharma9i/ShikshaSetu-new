...
export default function CircularsFilterBar() {
  ...
  const hasActiveFilters = search || category !== 'all' || role !== 'all'
  ...
  return (
    <div className="bg-white border border-light-gray/60 rounded-2xl p-5 shadow-sm space-y-4 font-body">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <label htmlFor="role" className="block text-xs font-semibold text-steel-gray uppercase tracking-wider mb-2 font-caption">
            Target Audience
          </label>
          <select
            id="role"
      ...
      {(hasActiveFilters || isPending) && (
        <div className="flex items-center justify-between pt-2 border-t border-light-gray/40">
          ...
          {hasActiveFilters && (
            <button
              onClick={handleReset}
              className="text-xs font-semibold text-danger hover:text-red-700 py-1.5 px-3 rounded-lg hover:bg-danger/5 transition-all cursor-pointer active:scale-95"
            >
              Clear All Filters
            </button>
          )}
