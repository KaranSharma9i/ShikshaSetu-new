...
Keep the existing UI and navigation flow intact. Implement email-based Sign In and verification code handling through Clerk.

After successful verification/authentication, navigate to the home route (/) to three different sections: (1) Students, (2) Teachers, and (3) Schools as discussed in @AGENTS.md . If not authenticated, show onboarding route (/onboarding(. If authenticated, show home route (/).)) 
